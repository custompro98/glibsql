...
pub fn main() {
  ...
  let statement =
    ...
    |> glibsql.with_query(
      "SELECT id, email, created_at, updated_at FROM users WHERE id = :id OR email = $email",
    )
    |> glibsql.with_argument(glibsql.NamedArgument(
      name: "email",
      value: glibsql.Text("tom@example.com"),
    ))
    |> glibsql.with_argument(glibsql.NamedArgument(
      name: "id",
      value: glibsql.Integer(1),
    ))
